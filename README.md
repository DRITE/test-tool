## Test-tool 
В будующем это будет мощный инструмент для проверки тестовых задач на собеседованиях.

### Запуск проекта
Прежде всего на твоём компуктере должна быть установлена нода. Для виндовс её можно скачать на официальном сайте.
Если что, у меня установлена v6.4.0. Совутую качать не ниже этой версии.
После установки ноды выполнить:

* `npm install`

Пока нет интеграции с сервером, запуск в dev режиме осуществляется:

* `npm run start:dev`

Эта команда запускает webpack-dev-server, который запускает webpack, который компилирует TypeScript в JavaScript и собирет это всё в один файл.
Кстати, чтобы посмотреть, что там в итоге запустилось, надо зайти на localhost:9000
...
from [`src/index.js`](src/old/index.js).

If you had previously enabled service workers in your production deployment and
have decided that you would like to disable them for all your existing users,
you can swap out the call to `registerServiceWorker()` in
[`src/index.js`](src/old/index.js) first by modifying the service worker import:
```javascript
...
in all web browsers. Service worker registration [won't be attempted](src/old/registerServiceWorker.js)
...
starting point, you can make use of the logic included in [`src/registerServiceWorker.js`](src/old/registerServiceWorker.js), which
